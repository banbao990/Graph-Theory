...
+ $w_{i,j}=g((v_i,v_j)),\quad if(v_i,v_j) \in E$

  $w_{i,j} =0,\qquad \qquad \quad else$
...
+ $b_{i,j}=1, \qquad if结点v_i是边e_k的直接前驱$

  $b_{i,j}=-1, \quad if结点v_i是边e_k的直接后继$
...
+ 对关联矩阵的列进行压缩的结果
+ 由两个 $m$ 维的向量 $A,B$ 组成
+ $e_k=(v_i,v_j) \Rightarrow A(k)=i,B(k)=j$
+ 可以增加一个向量 $C$ 用于存边权 $C(k)=g(e_k)$

### 1.2.5 正向表

+ 对邻接矩阵的行进行压缩的结果

+ 有向图的正向表由$1$个 $(n+1)$ 维的向量 $A$ 和一个 $m$ 维的向量 $B$ 组成

+ 集合 $B$ 中依次存放每个结点**直接后继**的编号

+ $A(i)$表示结点 $v_i$ 的第一个直接后继在 $B$ 中存放的位置

  约定 $A(n+1)=m+1$ (哨兵, 结束标记)

+ 可以增加一个向量 $C$ 用于存边权 $C(k)=g( (\arg\max_i A(i) \leq k),B(k))$
+ $d^+(v_i)=A(i+1)-A(i)$
+ $A(1)=1,A(i)=\sum_{j=1}^{i-1}d^+(v_i)+1$

### 1.2.6 反向表

+ 类似于正向表, 集合 $B$ 中存放**直接前驱**
...
+ 每个结点的所有**外邻点**组成一个单链表
  + 链表头指向第一个直接后继
  + 每个外邻点占一个链表元素结点,
  + 链表元素结点结构包括外邻点域 $a$ , 边权域 $b$ , 指针域 $c$

### 1.2.8 反向邻接表

+ 类似于正向邻接表, 每个节点的所有**内邻点**组成一个单链表
