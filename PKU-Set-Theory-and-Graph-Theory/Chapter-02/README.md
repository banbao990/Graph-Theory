...
+ **最短奇闭路径**, 断言其为奇回路
+ $证明:$
  + $设C是一条奇闭路径,设C_1是C上边数最少的一条奇闭路径,断言C_1为奇回路$
  + $否则C_1不是回路(奇数个结点),此时,C_1上有重复出现的结点v_0$
  + $在路径C_1上从v_0处将C_1分割为两个闭路径,其中必有一个为奇闭路径,与C_1极小性矛盾$
  + $C_1为奇回路$
...
+ **笛波滤恩序列**/**笛波滤恩图**
  ...
  + 有向边表示一个输出
    ...
    + 满足 $a_i=b_{i-1},\ \forall i \in [2,n-1] \cap N$
...
+ $任意强连通的竞赛图D(n \geq 3)必是有向H图$

+ $证明:$
  + $1.D中存在有向回路$
  + $2.若D中存在长度为k(3 \leq k < n-1)的有向回路C,必存在长度为k+1的有向回路$
    + $分类:C外是否存在结点u使得C上既有u的内邻点又有u的外邻点$
      + $1.有 \Rightarrow OK$
      + $2.无$
        + $V_1=\lbrace v:C上的点都是v的外邻点 \rbrace,V_2=\lbrace v:C上的点都是v的内邻点 \rbrace$
        + $由强连通性,\exists v_1 \in V_1,v_2 \in V_2,使得(v_2,v_1) \in E(G)$
        + $\Rightarrow OK$
...
+ **格雷码** ($Gray\ Code$)
  ...
  + 对于由 $2^n$ 个字长为 $n$ 的不同二进制数组成的循环序列,

    如果相邻的两个二进制数恰有一位数字不同,则称该循环序列为**格雷码** 

+ 每个格雷码对应着 $n$ 立方体一个 $H$ 回路
...
+ $2.1\ $
  ...
  $若从 v_0 开始沿着G 中的边(不重复)行走，无论走个结点处，都可以接着走遍没走过的边，$

  $然后回到v_0,则称v_0是可以任意行遍的.$

  $试证明:$

  + $v_0是可以任意行遍的 \Leftrightarrow G-v_0中无回路$

+ $2.2\ $

  $如何将9个\alpha,9个\beta,9个\gamma排成循环序列x_1x_2...x_{27},$

  $使得对于字母表\lbrace \alpha,\beta,\gamma \rbrace 上的任何字长为3的字w,$

  $都存在i(1 \leq i \leq 27),使得x_ix_{i+1}x_{i+2}=w(溢出取模)$

+ $2.3\ 设G 为n 阶无向简单图,边数m=\frac12(n-1)(n-2)+2$

  + $证明:$
    + $1.G是H图$
    + $2.举例说明,当m=\frac12(n-1)(n-2)+1时,G是非H图$

+ $2.4\ 若二部图G=(V_1,V_2,E)中,|V_1| \ne |V_2| \Rightarrow G是非H图$

+ $2.5\ $

  $对一个3 \times 3 \times 3的立方体,$

  $能否从一个角开始,通过所有27个1 \times 1 \times 1 \times 的小立方块一次,最后到达中心?试说明理由.$

+ $2.6\ $ 

  + $设G 是无向简单图,若从G 的任意一点v0出发,$

    $沿着G 的边随意走向任何尚未经过的结点,总能走出一条哈密顿道路,$

    $则称G是随意哈密顿图.$

  + $G是随意哈密顿图 \Leftrightarrow G是K_n,C_n 或K_{\frac{n}2,\frac{n}2}$ 。
...
+ $2.1\ $
  ...
  + $\Rightarrow:反证法$
    ...
    + $假设G-v_0中含有回路,设回路C',G'=G-E(C')(仅删去C'上的边)$

      $此时G'中所有结点的奇偶性与G相同(都是偶结点)$

      + $case\ 1:G'连通 \Rightarrow G'也是欧拉图$

        + $此时从v_0出发,在G'中行遍所有边(找一条欧拉闭链)必然回到v_0$
        + $由于v_0 \notin C',无法继续行遍C' \Rightarrow 无法行遍G \Rightarrow 矛盾$

      + $case\ 2:G'不连通$

        + $设G'有k(k \geq 2)个连通分支G_1,G_2,...,G_k,显然G_i(\forall i \in N \cap [1,k])都是欧拉图$

          $不妨设v_0 \in G_1,则从v_0出发,行遍G_1(找一条欧拉闭链)必然回到v_0$

          $由于v_0 \notin C' \Rightarrow C',G_i(\forall i \in N \cap [2,k])都无法行遍 \Rightarrow 无法行遍G \Rightarrow 矛盾$

  + $\Leftarrow$

    + $G为欧拉图,因此G可以表示为若干个边不交的回路的并, 即G=\bigcup\limits_{i=1}^dC_i$

      $由于G-v_0无回路,因此所有回路都经过v_0,此时从v_0出发,可以随意行遍C_i(1 \leq i \leq d),最终行遍G,回到v_0$

+ $2.2\ 笛波滤恩编码+欧拉闭链$

  + $9结点v \in \lbrace ab:a,b \in \lbrace \alpha,\beta,\gamma \rbrace \rbrace$
  + $27边$
...
+ $2.3\ $
  ...
  + $1\ $
    ...
    + $|E(K_n)|=\frac12n(n-1),m(G)-|E(K_n)|=n-3$

    + $G可由K_n删去(n-3)条边形成$

    + $在K_n中,任取两点u,v,d(u)=d(v)=n-1,d(u)+d(v)=2n-2$

      $\Rightarrow 在G中,任取两点u,v,d(u)+d(v) \geq (2n-2)-[(n-2)+2]=n$

      $\Rightarrow OK$

  + $2\ K_{n-1}+\lbrace v_0 \rbrace$

+ $2.4\ 不妨设|V_1| > |V_2|,若存在H回路,则必然有两个|V_1|中的结点相邻,与二部图矛盾$

+ $2.5\ 二部图G(V_1,V_2,E),|V_1|=14 \ne |V_2|=13$ 
  + $A:8个角 \sim B:12个棱中心 \sim C:6个面中心 \sim D:1个体心$
  + $V_1=A \cup C,V_2=B \cup D$
...
+ $2.6\ $
  ...
  + $\Rightarrow:分类讨论$
    ...
    + $断言:若G能走出随意哈密顿道路,G必然能走出随意哈密顿回路$

      + $设v_{i_1}v_{i_2}...v_{i_n}为G走出的一条随意哈密顿道路,此时v_{i_n}必然与v_{i_0}相邻$
        + $因为从v_{i_2}出发,可以走出v_{i_2}v_{i_3}...v_{i_n},由题设,必然由v_{i_n}与v_{i_1}相邻$

    + $G包含一条哈密顿回路C,不妨令C=v_nv_1v_2..v_n, \delta(G) \geq 2$

    + $case\ 1:G是 2-正则图$

      + $此时 G = C = C_n$

    + $case\ 2:G中存在结点v,使得d(v) \geq 3$

      + $case\ 2.1: G中包含三角形$

        + $不妨设三角形为v_1v_2v_3v_1$

        + $以下证明v_2与所有结点都相邻,即证v_2与v_k(\forall k \in [3,n] \cap N)相邻$

          + $v_2与v_1,v_3相邻$

          + $对k实施归纳$

            + $因为v_2v_kv_{k-1}v_{k-2}...v_3v_1v_nv_{n-1}v_{n-2}...v_{k+1}是一套哈密顿道路$

              $\Rightarrow v_2 与 v_{k+1}相邻$

        + $三角形v_2v_3v_4v_2,此时v_3与之前证明的v_2等价 \Rightarrow v_3与所有结点都相邻$

        + $三角形v_nv_1v_2v_n,此时v_1与之前证明的v_2等价 \Rightarrow v_1与所有结点都相邻$

        + $\Rightarrow 如果一个结点是某个三角形的顶点,则此节点与其他任意结点都相邻$

        + $v_1,v_2与其他任意结点都相邻$

          $ \Rightarrow v_1,v_2与其他任意结点v(\forall v \in V(G))都相邻$

          $\Rightarrow 任意结点v(\forall v \in V(G))其他任意结点都相邻$
      ...
      + $case\ 2.2 :G中不含三角形$
        ...
        + $断言:G中必含有四角形$
          ...
          + $由于G不只是C,G中必有回路C的弦,不妨设弦为v_1v_a(2 < a < n)$
          + $以下证明v_2与v_{a+1}相邻$
            + $v_2v_3...v_av_1v_nv_{n-1}...v_{a+1} \Rightarrow v_2 与 v_{a+1}相邻$
          + $从而G中含有四边形v_1v_2v_{a+1}v_av_1$

        + $不妨设四边形为v_1v_2v_3v_4v_1$

        + $下面证明$

          $ ①n为偶数$

          $②任意奇数编号结点与任意偶数编号结点相邻,任意奇偶性相同的结点不相邻$

          + $H道路:v_2v_3v_4v_1v_nv_{n-1}...v_5 \Rightarrow v_2与v_5相邻$

          + $此时v_2v_3v_4v_5v_2为四边形,同理有v_3与v_6相邻$

          + $总而言之:v_i与v_{i+3}相邻(\forall i \in N \cap [1,n],溢出取模)$

          + $证明一般情况,v_i与v_{i+2k-1}相邻,对k(k \geq 2)进行归纳$

            + $设v_i与v_{i+2l-1}相邻(\forall 1 \leq i \leq n,1 \leq l \leq k,k \geq 2)$

            + $l=1,2成立 \Rightarrow k =2 时成立$

            + $要证v_i与v_{i+2(k+1)-1}相邻,只需证明i=1时成立即可(图完全对称)$

              $即证:v_i与v_{i+2k-1}相邻 \Rightarrow v_1与v_{2(k+1)}相邻$

              + $H道路:v_1v_nv_{n-1}...v_{2k+3}v_{2k}v_{2k-1}...v_2v_{2k+1} \Rightarrow v_1与v_{2k+1}相邻$

          + $\Rightarrow 任意奇数标号结点与任意偶数标号结点相邻$

          + $由于G不包含三角形 \Rightarrow 任意奇数(偶数)标号结点其他任意奇数(偶数)标号结点不相邻$

          + $v_1与v_n相邻 \Rightarrow n为偶数$
